  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await fetch('/api/auth/register', {
        ...
        body: JSON.stringify({
          firstName: formData.get('firstName'), // Retrieve the first name value from the form data.
      ...
      if (!response.ok) {
        if (data.errors) {
          ...
          throw new Error(data.error || 'Registration failed'); // Throw a generic error if no specific errors are provided.
        }
    ...
    <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
      ...
      <div className="w-full max-w-md space-y-8">
        ...
        <form onSubmit={handleSubmit} className="space-y-6 bg-white p-8 rounded-xl shadow-sm">
          ...
          <div>
            ...
            <div className="mt-1 relative rounded-md shadow-sm">
              ...
              <input
                ...
                placeholder="John" // Placeholder text for first name input field.
              />
            </div>
          </div>
          ...
          >
            {isLoading ? 'Creating account...' : 'Create Account'} {/* Show loading indicator or button label */}
          </button>
        </form>
